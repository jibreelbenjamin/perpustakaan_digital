...
<x-app :page='$page'>
    <div class="max-w-4xl mx-auto py-5 sm:py-10 px-5 md:px-8 2xl:px-5">
      ...
      <div>
        <div class="mb-3">
          <!-- List -->
          <div id="akses_token-container" class="-mx-3">
...
function loadMoreData(reset = false) {
    if (reset) {
        offset = 0;
        allDataLoaded = false;
        document.getElementById('akses_token-container').innerHTML = '';
    ...
    setTimeout(() => {
        fetch(`{{ route('akses_token.load') }}?offset=${offset}&limit=${limit}&search=${encodeURIComponent(searchQuery)}`)
            .then(res => res.json())
            .then(data => {
                ...
                const container = document.getElementById('akses_token-container');
                data.forEach(item => {
                    container.insertAdjacentHTML('beforeend', `
                        <a class="mb-2 p-3 group flex gap-x-4 rounded-2xl hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800" href="{{ env('APP_URL') }}/akses_token/${item.id}/detail">
                            <div class="grow">
                                    ...
                                    <div>
                                        ...
                                        <ul class="flex flex-wrap">
                                            ...
                                            <li class="inline-block relative text-sm pe-4 last:pe-0 last-of-type:before:hidden before:absolute before:top-1/2 before:end-1.5 before:-translate-y-1/2 before:size-[3px] before:bg-gray-500 before:rounded-full dark:before:bg-neutral-500 ${
                                                item.status === 'kadaluarsa' ? 'text-red-500' : 
                                                item.status === 'aktif' ? 'text-green-500' : ''
                                            }">
