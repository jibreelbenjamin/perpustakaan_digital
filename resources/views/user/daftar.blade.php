...
function loadMoreData(reset = false) {
    ...
    setTimeout(() => {
        fetch(`/user/load?offset=${offset}&limit=${limit}&search=${encodeURIComponent(searchQuery)}`)
            .then(res => res.json())
            .then(data => {
                ...
                data.forEach(item => {
                    container.insertAdjacentHTML('beforeend', `
                        <a class="mb-2 p-3 group flex gap-x-4 rounded-2xl hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800" href="/user/${item.id_user}/detail">
                            <div class="grow">
...
function input(){
    ...
    fetch("https://perpustakaan_digital.test/user", {
        method: 'POST',
        headers: {
